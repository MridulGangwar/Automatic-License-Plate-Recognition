...
charModel = pickle.loads(open("output/char.cpickle", "rb").read())
digitModel = pickle.loads(open("output/digit.cpickle", "rb").read())
...
for imagePath in sorted(list(paths.list_images(args["images"]))):
	...
	lpd = LicensePlateDetector(image, minChars=7)
	plates = lpd.detect()
